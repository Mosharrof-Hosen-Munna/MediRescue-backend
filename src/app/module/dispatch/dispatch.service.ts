...
import type {
	...
	IUpdateDispatchActionPayload,
	IUpdateDispatchPayload,
...
const createDispatch = async (
	...
	const { emergencyRequestId, ambulanceId, driverId } = payload;

	const [emergencyRequest, ambulance, driver] = await Promise.all([
	...
	if (emergencyRequest.status !== "PENDING") {
		throw new Error("Only pending emergency requests can be dispatched");
	}
	...
	if (ambulance.driverId !== driverId) {
		throw new Error("Selected driver is not assigned to this ambulance");
	}

	if (ambulance.typeId !== emergencyRequest.ambulanceTypeId) {
		throw new Error(
	...
	if (existingDispatch) {
		throw new Error("This emergency request already has a dispatch");
	}
	...
	const result = await prisma.$transaction(async (tx) => {
		...
		await auditLogService.createAuditLog(tx, {
			...
			description: "Ambulance and driver assigned to emergency request",
		});
...
const getDispatches = async (query: IGetDispatchesQuery) => {
...
const updateDispatch = async (
	...
	if (payload.ambulanceId === undefined && payload.driverId === undefined) {
		throw new Error("At least one field is required to update dispatch");
	}
	...
	if (["COMPLETED", "CANCELLED"].includes(dispatch.status)) {
		throw new Error("Completed or cancelled dispatch cannot be updated");
	}

	const ambulanceId = payload.ambulanceId ?? dispatch.ambulanceId;

	const driverId = payload.driverId ?? dispatch.driverId;
	...
	if (driver.status !== "AVAILABLE" && driver.id !== dispatch.driverId) {
	...
	if (ambulance.typeId !== dispatch.emergencyRequest.ambulanceTypeId) {
		throw new Error(
			"Selected ambulance does not match the requested ambulance type",
		);
	}
	...
	const result = await prisma.$transaction(async (tx) => {
		...
		await auditLogService.createAuditLog(tx, {
			...
			oldValue,
			newValue: {
				ambulanceId: updatedDispatch.ambulanceId,
				driverId: updatedDispatch.driverId,
				status: updatedDispatch.status,
			},
			description: "Dispatch assignment updated",
		});
...
const updateDispatchAction = async (
	...
	if (dispatch.driver.userId !== userId) {
		throw new Error("You are not allowed to update this dispatch");
	}
	...
	if (payload.status === "reject" && !payload.rejectionReason) {
		throw new Error("Rejection reason is required when rejecting a dispatch");
	}
	...
	const result = await prisma.$transaction(async (tx) => {
		...
		if (payload.status === "accept") {
			...
			updatedDispatch = await tx.dispatch.update({
				...
				data: {
					...
					cancellationReason: payload.rejectionReason,
				},
				include: {
					emergencyRequest: {
						include: {
		...
		await auditLogService.createAuditLog(tx, {
			...
			newValue: {
				...
				cancellationReason: updatedDispatch.cancellationReason,
			},
			description:
