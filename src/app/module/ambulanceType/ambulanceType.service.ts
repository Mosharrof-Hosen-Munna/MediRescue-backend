...
import { ICreateAmbulanceTypePayload, IGetAmbulanceTypesQuery, IUpdateAmbulanceTypePayload } from "./ambulanceType.interface";
...
const updateAmbulanceType = async (
    ...
    const result = await prisma.$transaction(async (tx) => {
        const updatedAmbulanceType =
            await tx.ambulanceType.update({
                where: {
                    id,
                },
                data: {
...
const deleteAmbulanceType = async (
    id: string,
    adminUserId: string
) => {
    const ambulanceType = await prisma.ambulanceType.findUnique({
    ...
    const result = await prisma.$transaction(async (tx) => {
        const deletedAmbulanceType =
            await tx.ambulanceType.delete({
                where: {
                    id,
                },
                select: {
